...
import BulkOutboundManager from './components/BulkOutboundManager';
import ReturnManager from './components/returns/ReturnManager';
import ReturnQCManager from './components/ReturnQCManager';
import ReturnStockInManager from './components/ReturnStockInManager';
import TransitManager from './components/TransitManager';
import WorkOrderManager from './components/WorkOrderManager';
import ReportsManager from './components/ReportsManager';
import FBAReturnManager from './components/FBAReturnManager';
import BoxInventoryManager from './components/BoxInventoryManager';
import ReturnStockManager from './components/ReturnStockManager';
import DefectiveManager from './components/DefectiveManager';
    ...
    <div id="wms-app-viewport" className="min-h-screen bg-slate-100 flex font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        {activeMenu === '首页' ? (
              ...
              <div className="flex items-center gap-1.5 font-sans">
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      await outboundApi.batchPrintPickList(selectedIds);
                      showToast(`发货清单打印任务已发送到仓库打印机！共 ${selectedIds.length} 份。`, 'success');
                      fetchOrders();
                      setSelectedIds([]);
                    } catch (err) {
                      showToast('打印发货清单失败，请重试。', 'error');
                    }
                  }}
                  className="h-6.5 px-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 rounded font-semibold cursor-pointer"
                >
                ...
                <button
                  onClick={() => {
                    if (selectedIds.length === 0) {
                      showToast('请先勾选出库订单再导出面单附件。', 'error');
                      return;
                    }
                    const selectedOrders = orders.filter(o => selectedIds.includes(o.id));
                    const header = ['订单号', '客户', '物流渠道', '承运商', '收件人', '产品数量'];
                    const rows = selectedOrders.map(o => [
                      o.orderNo, o.customerName || '', o.logisticsChannelName || '',
                      o.carrierName || '', o.recipient, String(o.totalQty)
                    ]);
                    downloadCSV([header, ...rows], `面单附件_${new Date().toISOString().slice(0, 10)}.csv`);
                    showToast(`已导出 ${selectedIds.length} 条面单附件记录。`, 'success');
                  }}
                  className="h-6.5 px-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 rounded font-semibold flex items-center gap-0.5 cursor-pointer"
                >
                  <span>导出附件</span>
                ...
                <button
                  onClick={() => {
                    const header = ['订单号', '客户', '物流渠道', '承运商', '状态', '收件人', '产品数量', '创建时间'];
                    const rows = orders.map(o => [
                      o.orderNo, o.customerName || '', o.logisticsChannelName || '',
                      o.carrierName || '', o.status, o.recipient,
                      String(o.totalQty), o.createdTime || ''
                    ]);
                    downloadCSV([header, ...rows], `出库订单_${new Date().toISOString().slice(0, 10)}.csv`);
                    showToast('已导出当前搜索条件的出库订单明细。', 'success');
                  }}
                  className="h-6.5 px-2.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 rounded font-semibold flex items-center gap-1 cursor-pointer"
                >
          ...
          /* Fallback page for unmatched menu items */
          <div className="flex-1 bg-slate-50 p-6 flex flex-col items-center justify-center font-sans text-center select-none">
            ...
            <h2 className="text-base font-bold text-slate-800 mb-1">{activeMenu}</h2>
            <p className="text-slate-400 text-xs max-w-sm leading-relaxed">暂无数据</p>
          </div>
        )}
