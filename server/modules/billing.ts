...
export function registerBillingRoutes(router: Router): void {
  router.get('/billing-rules', requireAuth, async (req: any, res) => {
    ...
    if (hasDb) {
      ...
      } catch (err) { console.error('Prisma billing rules fetch error:', err); }
    }
    res.json([
  ...
  router.post('/billing-rules', requireAuth, requireRole('ADMIN', 'SUPER_ADMIN'), async (req: any, res) => {
  ...
  router.put('/billing-rules/:id', requireAuth, requireRole('ADMIN', 'SUPER_ADMIN'), async (req: any, res) => {
  ...
  router.delete('/billing-rules/:id', requireAuth, requireRole('ADMIN', 'SUPER_ADMIN'), async (req: any, res) => {
    const hasDb = await checkDbConnection();
    if (hasDb) {
      const prisma = getPrisma();
      try {
  ...
  router.get('/billing-records', requireAuth, async (req: any, res) => {
    const user = req.user;
    const hasDb = await checkDbConnection();
    if (hasDb) {
      const prisma = getPrisma();
      try {
        const where: any = {};
        if (user && (user.role || '').toUpperCase() === 'CLIENT') where.customerId = user.customerId;
        const records = await prisma.billingRecord.findMany({ where });
        return res.json(records);
      } catch (err) { console.error('Prisma billing records fetch error:', err); }
    }
    res.json([{ id: 'br_rec_1', customerId: user.customerId || 'cust_1', orderId: 'ord_spec_1', type: 'OUTBOUND', amount: 2.5, currency: 'USD', status: 'UNPAID', createdAt: new Date() }]);
  });
  ...
  router.get('/invoices', requireAuth, async (req: any, res) => {
    ...
    if (hasDb) {
      ...
      } catch (err) { console.error('Prisma invoices fetch error:', err); }
    }
    res.json([]);
  });

  router.post('/billing-records/generate', requireAuth, requireRole('ADMIN', 'SUPER_ADMIN'), async (req: any, res) => {
    const { customerId, warehouseId, periodStart, periodEnd } = req.body;
    if (!periodStart || !periodEnd) return res.status(400).json({ error: 'periodStart and periodEnd are required (ISO date strings)' });
    try {
  ...
  router.post('/invoices/generate', requireAuth, requireRole('ADMIN', 'SUPER_ADMIN'), async (req: any, res) => {
  ...
  router.post('/invoices/:id/mark-paid', requireAuth, requireRole('ADMIN', 'SUPER_ADMIN'), async (req: any, res) => {
    const result = await markInvoicePaid(req.params.id);
    if (!result.success) return res.status(400).json({ error: result.error });
    return res.json({ status: 'success' });
  });

  router.post('/invoices/:id/void', requireAuth, requireRole('ADMIN', 'SUPER_ADMIN'), async (req: any, res) => {
    const result = await voidInvoice(req.params.id);
    if (!result.success) return res.status(400).json({ error: result.error });
    return res.json({ status: 'success' });
  });

  router.post('/invoices/:id/recalculate', requireAuth, requireRole('ADMIN', 'SUPER_ADMIN'), async (req: any, res) => {
